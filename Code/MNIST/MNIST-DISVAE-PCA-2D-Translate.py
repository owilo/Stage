...
plt.scatter(X_pca_src_centroid[0], X_pca_src_centroid[1], marker="x", color="red", s=100, label="Centroïde source")
plt.scatter(X_pca_dst_centroid[0], X_pca_dst_centroid[1], marker="x", color="blue", s=100, label="Centroïde destination")

plt.arrow(X_pca_src_centroid[0], X_pca_src_centroid[1],
          X_pca_dst_centroid[0] - X_pca_src_centroid[0],
          X_pca_dst_centroid[1] - X_pca_src_centroid[1],
          color="black", width=0.01, head_width=0.2, length_includes_head=True, label="Translation (centres)")

for i in range(10):
    src = X_pca_original[digits[src_class][i]]
    dst = X_pca_translated[i]
...
plt.title(f"PCA : Translation de {src_class} vers {dst_class}")
plt.legend()
plt.tight_layout()
plt.savefig("./Results/mnist-translation-pca.png")