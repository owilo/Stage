...
axes[0].plot(mean_encoded_dst[0] - mean_encoded_src[0], color="gray", ls="--", lw=1, label=f"Différence (translation {src_class} → {dst_class})")
axes[0].plot(mean_encoded_src[0], color="red", lw=3, label=f"Centroïde source ({src_class})")
axes[0].plot(mean_encoded_dst[0], color="blue", lw=3, label=f"Centroïde destination ({dst_class})")
axes[0].set_title("Centroïdes")
axes[0].legend()

axes[1].plot(X_encoded_valid[0] - mean_encoded_src[0], color="gray", ls="--", lw=1, label=f"Différence (écart au centroïde source {src_class})")
axes[1].plot(mean_encoded_src[0], color="red", ls="--", lw=2, label=f"Centroïde source ({src_class})")
axes[1].plot(X_encoded_valid[0], color="darkred", lw=3, label=f"Chiffre source ({src_class})")
axes[1].set_title("Centroïde source et Chiffre source")
axes[1].legend()

axes[2].plot(translated1[0] - mean_encoded_dst[0], color="gray", ls="--", lw=1, label=f"Différence (écart au centroïde destination {dst_class})")
axes[2].plot(mean_encoded_dst[0], color="blue", ls="--", lw=2, label=f"Centroïde destination ({dst_class})")
axes[2].plot(translated1[0], color="darkblue", lw=3, label=f"Chiffre translaté ({dst_class})")
axes[2].set_title("Centroïde destination et Chiffre translaté")
axes[2].legend()

axes[3].plot(translated2[0] - translated1[0], color="gray", ls="--", lw=1, label=f"Différence")
axes[3].plot(translated1[0], color="darkblue", lw=3, label=f"Chiffre translaté ({dst_class})")
axes[3].plot(translated2[0], color="#32CD32", ls="--", lw=2, label=f"Chiffre décodé & translaté ({dst_class})")
