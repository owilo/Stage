...
decoder = Model(decoder_input, x, name=f"h-decoder-{latent_dim}")
z_decoded = decoder(z)
...
encoder = Model(input_img, z_mu, name=f"h-encoder-{latent_dim}")
encoder.summary()
...
vae.fit(x = X_split1, y = None, shuffle = True, epochs = num_epochs, batch_size = batch_size, validation_split = 0.1)

encoder.save(f"./Models/DISVAE/mnist-{latent_dim}-h-encoder.keras")
decoder.save(f"./Models/DISVAE/mnist-{latent_dim}-h-decoder.keras")