...
def process_class_translations(source_class, z_test, y_test, z_train, y_train, result_dir):
    ...
    compute_confusion_matrix(
        ...
        certainties,
        np.arange(10),
        result_dir / f"mnist-transform-confusion-{source_class}.png",
        f"Transformation de {source_class} → j"
    )
    
    return forward_cm, x_decoded, y_dst, certainties

def process_inverse_translations(x_decoded_first, y_dst_labels, source_class, z_train, y_train):
...
for src_class in range(10):
    ...
    forward_cm, x_decoded, y_dst, forward_certainties = process_class_translations(
        src_class, z_test, y_test, z_train, y_train, FOLDER
    )
    ...
    inverse_cm_class, inverse_certainties = process_inverse_translations(
        x_decoded, y_dst, src_class, z_train, y_train
    )
...
compute_confusion_matrix(
    ...
    np.arange(10),
    FOLDER / "mnist-transform-confusion.png",
    "Transformation des classes i → j"
)
...
compute_confusion_matrix(
    ...
    np.arange(10),
    FOLDER / "mnist-inverse-transform-confusion.png",
    "Transformation inverse des classes i → j"
)