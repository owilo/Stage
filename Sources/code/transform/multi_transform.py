...
def generate_and_save_grids(x_decoded, filename_suffix):
    ...
    for src_class in range(10):
        ...
        file = cache.RESULTS_FOLDER / "TransformGrids" / f"mnist-{filename_suffix}-{src_class}.png"
        plt.savefig(file, bbox_inches='tight')
        plt.close(fig)
        print(f"Sauvegarde du fichier {file}")

generate_and_save_grids(x_decoded, "transform-grid")
generate_and_save_grids(x_reconstructed, "inverse-transform-grid")